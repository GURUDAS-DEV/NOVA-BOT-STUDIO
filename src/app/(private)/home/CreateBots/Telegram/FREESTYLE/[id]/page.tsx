...
const BOTFATHER_STEPS = [
  ...
  "Copy the generated token from BotFather.",
  "In next step, paste token and validate it.",
];

const Step1BotFatherGuide = ({
  checked,
  onChange,
}: {
  checked: boolean;
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Complete these steps to generate your Telegram bot token.
        </p>
      </div>
      ...
      <div className="rounded-md p-4 flex items-start gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
        ...
        <div className="text-sm text-amber-800 dark:text-amber-100">
          Security tip: keep your token private and never post it publicly.
        </div>
      </div>
      ...
      <label className="flex items-start gap-2 cursor-pointer text-sm text-gray-700 dark:text-gray-200">
        ...
        />
        I understand the BotFather steps and I am ready to validate my token.
      </label>
    </div>
  );
};

const Step2TokenValidation = ({
  token,
  status,
  message,
  isTesting,
  ...
  const tokenPreview = useMemo(() => {
    ...
    if (v.length <= 12) return "Provided";
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Backend will verify if the provided Telegram bot token is valid.
        </p>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-xs text-gray-600 dark:text-gray-400">
          Current token preview: {tokenPreview}
        </p>
        <Button
          onClick={onValidate}
    ...
    <div className="space-y-4">
      ...
      {value === "Other" && (
        <div className="w-full flex justify-center flex-col">
          <label className="font-outfit text-black dark:text-white pb-0.5 text-sm font-medium pl-1">
...
const Step6Scraping = ({
  ...
  const allConfirmationsChecked = ownershipConfirmed && permissionGranted && termsRead;
  const canScrape =
    scrapingEnabled && isValidUrl(websiteUrl) && allConfirmationsChecked && !scrapingComplete;

  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">Website Scraping</h3>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
      ...
      <div className="flex items-center space-x-2">
        ...
        />
        <label htmlFor="scraping-enabled" className="text-sm font-medium cursor-pointer text-gray-900 dark:text-white">
      ...
      {scrapingEnabled && (
        <>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-900 dark:text-white">Website URL</label>
            <Input
              ref={websiteUrlRef}
              type="url"
          ...
          <div className="space-y-3 p-4 rounded-md bg-gray-50 dark:bg-stone-800/50 border border-gray-200 dark:border-stone-700">
            <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">Required Confirmations</h4>
            ...
            <div className="flex items-start space-x-2">
              <input
                ...
                disabled={scrapingComplete}
              />
              <label htmlFor="ownership-confirmed" className="text-sm cursor-pointer text-gray-900 dark:text-white">
            ...
            <div className="flex items-start space-x-2">
              <input
                ...
                disabled={scrapingComplete}
              />
              <label htmlFor="permission-granted" className="text-sm cursor-pointer text-gray-900 dark:text-white">
            ...
            <div className="flex items-start space-x-2">
              <input
                ...
                disabled={scrapingComplete}
              />
              <label htmlFor="terms-read" className="text-sm cursor-pointer text-gray-900 dark:text-white">
          ...
          {scrapingError && (
            <div className="rounded-md border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/30 p-3">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-red-900 dark:text-red-200">Scraping Error</p>
                  <p className="text-sm text-red-800 dark:text-red-300 mt-1">{scrapingError}</p>
                </div>
              </div>
            </div>
          )}
...
const Step7Behavior = ({
  ...
  const enhanceTextWithAI = async (): Promise<void> => {
    ...
    if (description.trim().length <= 100) {
      toast.error("Please enter atleast 100 characters in the description to enhance.");
      return;
    }
    try {
      setIsEnhancing(true);
  ...
  return (
    <div className="space-y-6">
        ...
        {validationError && (
          <div className="rounded-md border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/30 p-3">
            <div className="flex items-start gap-2">
              <div className="text-red-600 dark:text-red-400 mt-0.5">⚠</div>
              <div>
                <p className="text-sm font-semibold text-red-900 dark:text-red-200">Validation Error</p>
                <p className="text-sm text-red-800 dark:text-red-300 mt-1">{validationError}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {pendingEnhancedText && (
          <div className="rounded-md border border-gray-200 dark:border-stone-700 bg-gray-50 dark:bg-stone-900/70 p-3 space-y-2">
            <div className="flex items-start justify-between gap-3">
              <div className="text-sm font-semibold text-gray-900 dark:text-white">Review enhanced text</div>
            ...
            <p className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{pendingEnhancedText}</p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2">
        <label className="text-sm font-medium text-gray-900 dark:text-white">Additional Information</label>
        <Textarea
...
const Step8Examples = ({
  ...
  const updateExample = (index: number, field: "question" | "answer", value: string) => {
    onChange(examples.map((ex, idx) => (idx === index ? { ...ex, [field]: value } : ex)));
  };
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          ...
          <span className="text-yellow-600 dark:text-yellow-300 font-medium">
            <br></br>Please Provide Atleast 5 Example for better working of your Bot!
          </span>
        </p>
      </div>
          ...
          >
            <CardHeader className="pb-3">
              <div className="flex items-center dark:text-white text-black justify-between">
                ...
                {examples.length > 1 && (
                  <Button size="icon-sm" variant="ghost" onClick={() => removeExample(index)}>
    ...
    <div className="space-y-6">
      ...
      {enabled && (
        <>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-900 dark:text-white">API Usage Rules</label>
            <Textarea
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-900 dark:text-white">Response From Given API</label>
            <Textarea
              disabled={true}
          ...
          <div className="space-y-2">
            <Button onClick={onRunTest} variant="outline" className="w-full dark:bg-black bg-white dark:text-white text-black">
              Test API Connection
            </Button>
            {testResult && (
              <p
              ...
              >
                {testResult.includes("Succeeded") ? "API Test Succeeded!" : `API Test Failed: ${testResult}`}
              </p>
            )}
          </div>
        </>
  ...
  return (
    <div className="rounded-md p-4 mb-4 flex items-start gap-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
      ...
      <div className="text-sm text-yellow-800 dark:text-yellow-100">
        <div className="font-semibold mb-1">Important: Provide clear bot details</div>
        <div>
          Please fill out all fields carefully. The more accurate and detailed Information you provide, the better the bot responses and behavior will be. Do not include reckless, sensitive, or unnecessary information in these fields.
        </div>
      </div>
    </div>
  );
...
export default function CreateTelegramBotWizard() {
  ...
  const fetchBotStructure = async (): Promise<void> => {
    try {
      ...
      if (response.ok) {
        setBotName(data?.bot?.botName || "Telegram Bot");
        setDoesFound(true);
      } else {
        setDoesFound(false);
      }
  ...
  const validateToken = async (): Promise<void> => {
    const token = wizardState.telegramBotToken.trim();
    ...
    if (!TOKEN_FORMAT_REGEX.test(token)) {
      toast.error("Token format looks invalid.");
      return;
    }

    try {
      setIsTestingToken(true);
      updateState("tokenStatus", "running");
      updateState("tokenValidationMessage", "");

      const encodedToken = encodeURIComponent(token);
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/Telegram/ValidateBotToken/${encodedToken}`,
        {
          method: "GET",
          credentials: "include",
        }
      ...
      if (!isValid) {
        const message = result?.reason || result?.message || "Token validation failed.";
        updateState("tokenStatus", "invalid");
        updateState("tokenValidationMessage", message);
        toast.error(message);
        return;
      }

      updateState("tokenStatus", "valid");
      updateState("tokenValidationMessage", result?.message || "Token validated successfully.");
      toast.success("Token validated successfully.");
    } catch (error) {
      console.error("Token validation error:", error);
      updateState("tokenStatus", "invalid");
      updateState("tokenValidationMessage", "Validation service unavailable. Please try again.");
      toast.error("Could not validate token right now.");
    } finally {
  ...
  const validateExamples = async (): Promise<void> => {
    try {
      ...
      if (wizardState.examples.length < 5) {
      ...
      if (realData?.isValid || realData?.result === true) {
        setWizardState((prev) => ({
          ...prev,
        ...
        setWizardState((prev) => ({
          ...
          examplesValidated: false,
          examplesValidationError:
            realData?.reasons || realData?.reason || "Examples validation failed due to unknown reasons.",
        }));
  ...
  const scrapeWebsite = async (): Promise<void> => {
    ...
    if (!wizardState.ownershipConfirmed || !wizardState.permissionGranted || !wizardState.termsRead) {
    ...
    try {
      ...
      if (!response.ok) {
        setWizardState((prev) => ({
          ...prev,
          scrapingComplete: false,
          scrapingError: data?.message || "Failed to scrape website. Please check the URL and try again.",
        }));
      ...
      setWizardState((prev) => ({
        ...prev,
        scrapingComplete: false,
        scrapingError: "An unexpected error occurred while scraping. Please try again.",
      }));
  ...
  const runApiTest = async (): Promise<void> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.error("API Test Failed! Check your Endpoint or give permission to access this API on server.");
        return;
      }
      setWizardState((prev) => ({
        ...prev,
        apiTestPassed: true,
        responseFormat: data ? JSON.stringify(data, null, 2) : prev.responseFormat,
        apiTestResult: `API Test Succeeded: Received response ${JSON.stringify(data)}`,
      }));
  ...
  const validateTextWithAPI = async (): Promise<boolean> => {
    try {
      ...
      setValidationError(newData.reason || "Validation failed. Please check your input.");
      return false;
    } catch (error) {
      console.error("Error validating text:", error);
      setValidationError("An error occurred while validating. Please try again.");
      return false;
    } finally {
      setIsValidating(false);
    }
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 1:
        return wizardState.botFatherGuideRead;
      case 2:
        return wizardState.telegramBotToken.trim().length > 0 && wizardState.tokenStatus === "valid";
      case 3:
      ...
      case 5:
        return (
          ...
          (wizardState.websiteType !== "Other" || wizardState.otherWebsiteType.trim().length >= 3)
        );
      case 6:
      ...
      case 8:
        return wizardState.examples.some((ex) => ex.question.trim() && ex.answer.trim()) && wizardState.examplesValidated;
      case 9:
          ...
          <Card className="bg-white dark:bg-stone-900 border-gray-200 dark:border-stone-800 shadow-lg">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-lg text-gray-700 dark:text-gray-300 mb-8">
                Your bot <span className="font-semibold text-gray-900 dark:text-white">{botName}</span> is now ready to serve your customers.
              </p>
        ...
        <div className="max-w-2xl w-full">
          <Card className="bg-white dark:bg-stone-900 border-gray-200 dark:border-stone-800">
            <CardContent className="p-8 text-center">
              <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mb-2">Bot Not Found</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
                We could not find a bot with the id <span className="font-medium text-gray-900 dark:text-white">{botId || "(unknown)"}</span>.
              </p>

              <p className="text-sm text-gray-700 dark:text-gray-300 mb-6">
                You can configure this bot from the Manage page or create a new bot to get started.
              </p>
    ...
    <div className="min-h-screen bg-pink-50 dark:bg-stone-950 py-8 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="w-full font-outfit">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">SETUP YOUR BOT :</h1>
          <h2 className="text-2xl text-gray-900 dark:text-white mb-2">
            {botName ? `Configuring Bot - ${botName}` : "Creating a New Telegram Free-Style Bot"}
          </h2>
        </div>

        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Create Telegram Free-Style Bot</h1>
        ...
        <Card className="bg-white dark:bg-stone-900 border-gray-200 dark:border-stone-800">
          <CardContent className="pt-6">
            ...
            {currentStep === 1 && (
              <Step1BotFatherGuide
                checked={wizardState.botFatherGuideRead}
                onChange={(val) => updateState("botFatherGuideRead", val)}
              />
            )}

            {currentStep === 2 && (
              <Step2TokenValidation
                token={wizardState.telegramBotToken}
                status={wizardState.tokenStatus}
                message={wizardState.tokenValidationMessage}
                isTesting={isTestingToken}
                onTokenChange={(val) => {
                  updateState("telegramBotToken", val);
                  updateState("tokenStatus", "notTested");
                  updateState("tokenValidationMessage", "");
                }}
            ...
            {currentStep === 3 && (
              <Step3BotType value={wizardState.botType} onChange={(val) => updateState("botType", val)} />
            )}
            ...
            {currentStep === 6 && (
              <Step6Scraping
                ...
                onScrapingEnabledChange={(val) => updateState("scrapingEnabled", val)}
                onWebsiteUrlChange={(val) => updateState("websiteUrl", val)}
                onOwnershipConfirmedChange={(val) => updateState("ownershipConfirmed", val)}
                onPermissionGrantedChange={(val) => updateState("permissionGranted", val)}
            ...
            {currentStep === 7 && (
              <Step7Behavior
                ...
                onOwnerInformationChange={(val) => updateState("OwnerInformation", val)}
                onAdditionalInformationChange={(val) => updateState("additionalInformation", val)}
            ...
            {currentStep === 9 && (
              <Step9API
                ...
                onApiUsageRulesChange={(val) => updateState("apiUsageRules", val)}
        ...
        <div className="flex items-center justify-between mt-6">
          <Button
            onClick={handleBack}
            variant="outline"
            disabled={currentStep === 1 || isValidating || isTestingToken}
            className="bg-white dark:bg-stone-900 text-gray-900 dark:text-white border-gray-300 dark:border-stone-700 hover:bg-gray-100 dark:hover:bg-stone-800"
          >
            Back
          </Button>
          <Button
            onClick={handleNext}
            disabled={!validateStep(currentStep) || isValidating || isTestingToken}
            className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:bg-gray-800 dark:hover:bg-gray-100"
          >
