...
const BOTFATHER_STEPS = [
  ...
  "Set your bot display name and username (must end with 'bot').",
  "Copy the HTTP API token generated by BotFather.",
  "Paste the token here and validate it before continuing.",
...
export default function ControlledBotBuilder() {
  ...
  const [botFatherGuideRead, setBotFatherGuideRead] = useState<boolean>(false);
  const [telegramBotToken, setTelegramBotToken] = useState<string>("");
  const [tokenValidationStatus, setTokenValidationStatus] = useState<"notTested" | "running" | "valid" | "invalid">("notTested");
  const [tokenValidationMessage, setTokenValidationMessage] = useState<string>("");
  const [validatedBotTokens, setValidatedBotTokens] = useState<string[]>([]);

  const normalizedToken = telegramBotToken.trim();
  const isTokenValidated =
    tokenValidationStatus === "valid" &&
    normalizedToken.length > 0 &&
    validatedBotTokens.includes(normalizedToken);
  ...
  const validateTelegramToken = async () => {
    ...
    if (!TOKEN_FORMAT_REGEX.test(token)) {
      ...
      setTokenValidationMessage("Token format is invalid. Please check the token from BotFather.");
      toast.error("Token format looks invalid.");
      return;
    }

    try {
      setTokenValidationStatus("running");
      setTokenValidationMessage("");

      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/Telegram/ValidateBotToken/${encodeURIComponent(token)}`,
        {
          method: "GET",
      ...
      if (!response.ok) {
        const message = data?.message || "Invalid Telegram bot token.";
        setTokenValidationStatus("invalid");
      ...
      setTokenValidationMessage(data?.message || "Token validated successfully.");
      setValidatedBotTokens((prev) => (prev.includes(token) ? prev : [...prev, token]));
      toast.success("Telegram bot token validated.");
    } catch (error) {
      console.error("Error validating Telegram token:", error);
      setTokenValidationStatus("invalid");
      setTokenValidationMessage("Unable to validate token right now. Please try again.");
      toast.error("Token validation service is unavailable.");
    }
  };
  ...
  const createControlledStyleBot = async () => {
    if (!isTokenValidated) {
      toast.error("Please validate your Telegram bot token before creating this bot flow.");
      return;
    }
    ...
    try{
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/bot/setupWebsiteControlledStyleBotConfig`, {
        ...
        body: JSON.stringify({ bot, botId, botToken: normalizedToken, botTokens: validatedBotTokens }),
        credentials : 'include',
      })
  ...
  if (!isTokenValidated) {
    return (
      <div className="min-h-screen w-full bg-stone-100 dark:bg-stone-900 text-stone-900 dark:text-white font-inter px-6 py-8">
        <Toaster richColors position="top-right" />
        <div className="mx-auto w-full max-w-4xl rounded-2xl border border-stone-200 dark:border-stone-800 bg-white/90 dark:bg-stone-900/70 backdrop-blur p-6 md:p-8 shadow-xl space-y-6">
          <div className="space-y-2">
            <h1 className="text-2xl md:text-3xl font-outfit font-bold">Telegram Controlled Style Setup</h1>
            <p className="text-sm text-stone-600 dark:text-stone-300">
              Before building your Telegram flow, generate and validate your bot token using BotFather.
            </p>
          </div>

          <div className="rounded-xl border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-800/40 p-4 space-y-3">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <h2 className="text-base font-outfit font-semibold">How to Generate Telegram Bot Token</h2>
              <a
            ...
            <ol className="space-y-2 text-sm text-stone-700 dark:text-stone-300 list-decimal pl-5">
            ...
            <label className="flex items-center gap-2 text-sm pt-2">
              <input
                type="checkbox"
                checked={botFatherGuideRead}
                onChange={(e) => setBotFatherGuideRead(e.target.checked)}
                className="h-4 w-4"
              />
              I have completed or understood the BotFather steps.
            </label>
          </div>
          ...
          <div className="rounded-xl border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-800/40 p-4 space-y-3">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setTokenValidationStatus("notTested");
                setTokenValidationMessage("");
              }}
              placeholder="123456789:AA..."
              className="w-full bg-white dark:bg-stone-900 border border-stone-300 dark:border-stone-700 rounded-lg px-3 py-2 text-sm"
            />
            <div className="flex flex-col sm:flex-row gap-3">
              <Button
                onClick={validateTelegramToken}
                disabled={tokenValidationStatus === "running" || !botFatherGuideRead || !normalizedToken}
                className="bg-blue-600 hover:bg-blue-700 text-white"
              >
                {tokenValidationStatus === "running" ? "Validating..." : "Validate Telegram Token"}
              </Button>
              {tokenValidationStatus === "valid" && (
                <div className="text-sm text-emerald-600 dark:text-emerald-400 flex items-center">Token validated</div>
              )}
            </div>

            {tokenValidationMessage && (
              <p className={`text-xs ${tokenValidationStatus === "valid" ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
                {tokenValidationMessage}
              </p>
            )}

            {validatedBotTokens.length > 0 && (
              <div className="rounded-lg border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 p-3">
                <p className="text-xs text-stone-500 dark:text-stone-400 mb-2">Validated token history array</p>
                <div className="space-y-1">
                  {validatedBotTokens.map((token) => (
                    <p key={token} className="text-xs text-stone-700 dark:text-stone-200 font-mono">
                      {maskToken(token)}
                    </p>
                  ))}
                </div>
